# ToDo_ok: 1 - criar um teste que adicione um usuário
# ToDo_ok: 2 - realizar um login e extrair o token da resposta
...
def teste_incluir_usuario():
    ...
    mensagem_esperada = "5222428"

    # Executa
    resultado_obtido = requests.post(
        url=urlbase,
        headers=headers,
        data=open('F:\\projetoPython\\134inicial-a\\vendors\\json\\add_user.json')
    )
...
def teste_login_user():
    ...
    resultado_obtido = requests.get(
        data=open('F:\\projetoPython\\134inicial-a\\vendors\\json\\add_login.json'),
